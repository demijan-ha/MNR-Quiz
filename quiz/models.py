...
class UserProfile(models.Model):
    user = models.OneToOneField(settings.AUTH_USER_MODEL, related_name='profile', verbose_name=_("Profile"), on_delete=models.CASCADE)
...
class Quiz(models.Model):
    ...
    slug = models.SlugField(max_length=60, blank=False, help_text=_("a user friendly url"),
    ...
    answers_at_end = models.BooleanField(blank=False, default=False, help_text=_(
        "Correct answer is NOT shown after question. Answers displayed at the end."), verbose_name=_("Answers at end"))
    exam_paper = models.BooleanField(blank=False, default=False, help_text=_(
        "If yes, the result of each attempt by a user will be stored. Necessary for marking."),
                                     verbose_name=_("Exam Paper"))
    single_attempt = models.BooleanField(blank=False, default=False, help_text=_(
        "If yes, only one attempt by a user will be permitted. Non users cannot sit this exam."),
                                         verbose_name=_("Single Attempt"))
    pass_mark = models.SmallIntegerField(blank=True, default=0, verbose_name=_("Pass Mark"),
                                         help_text=_("Percentage required to pass exam."),
                                         validators=[MaxValueValidator(100)])
    success_text = models.TextField(blank=True, help_text=_("Displayed if user passes."),
                                    verbose_name=_("Success Text"))
    fail_text = models.TextField(verbose_name=_("Fail Text"), blank=True, help_text=_("Displayed if user fails."))
    draft = models.BooleanField(blank=True, default=False, verbose_name=_("Draft"), help_text=_(
    ...
    def save(self, force_insert=False, force_update=False, *args, **kwargs):
        ...
        self.slug = ''.join(letter for letter in self.slug if letter.isalnum() or letter == '-')
...
class AnonSittingManager(models.Manager):

    def new_sitting(self, uuid, quiz):
        if quiz.random_order is True:
        ...
        new_sitting = self.create(uuid=uuid,
                                  quiz=quiz,
                                  question_order=questions,
                                  question_list=questions,
                                  incorrect_questions="",
    ...
    def user_sitting(self, uuid, quiz):
        try:
            sitting = self.get(uuid=uuid, quiz=quiz, complete=False)
        except AnonSitting.DoesNotExist:
            sitting = self.new_sitting(uuid, quiz)
        except AnonSitting.MultipleObjectsReturned:
            sitting = self.filter(uuid=uuid, quiz=quiz, complete=False)[0]
        return sitting
...
class Sitting(models.Model):
    ...
    incorrect_questions = models.CharField(validators=[validate_comma_separated_integer_list], max_length=1024,
                                           blank=True, verbose_name=_("Incorrect questions"))
...
class AnonSitting(models.Model):
    ...
    question_list = models.CharField(validators=[validate_comma_separated_integer_list], max_length=1024, verbose_name=_("Question List"))
    incorrect_questions = models.CharField(validators=[validate_comma_separated_integer_list], max_length=1024, blank=True, verbose_name=_("Incorrect questions"))
    current_score = models.IntegerField(default=0, verbose_name=_("Current Score"))
    complete = models.BooleanField(default=False, blank=False, verbose_name=_("Complete"))
    user_answers = models.TextField(blank=True, default='{}', verbose_name=_("User Answers"))
    start = models.DateTimeField(auto_now_add=True, verbose_name=_("Start"))
    end = models.DateTimeField(null=True, blank=True, verbose_name=_("End"))
    objects = AnonSittingManager()
...
class AnonProgress(models.Model):
    anon_user = models.OneToOneField(AnonSitting, on_delete=models.CASCADE)
    score = models.CharField(validators=[validate_comma_separated_integer_list], max_length=1024,
                             verbose_name=_("Score"))
    correct_answer = models.CharField(max_length=10, verbose_name=_('Correct Answers'))
    wrong_answer = models.CharField(max_length=10, verbose_name=_('Wrong Answers'))
    objects = AnonProgressManager()
    ...
    def __str__(self):
        return str(self.anon_user.uuid) + ' - ' + self.score
...
class Question(models.Model):
    ...
    content = models.CharField(max_length=1000, blank=False, help_text=_("Enter the question text!"), verbose_name=_('Question'))

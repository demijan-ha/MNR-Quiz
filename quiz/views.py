...
from .models import Quiz, Category, Progress, Sitting, Question, AnonSitting, AnonProgress, User
...
class QuizTake(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.logged_in_user:
            ...
            self.sitting = AnonSitting.objects.user_sitting(uuid=self.uuid, quiz=self.quiz)
    ...
    def form_valid_user(self, form):
        ...
        if self.logged_in_user:
            ...
            anon_progress, c = AnonProgress.objects.get_or_create(anon_user=self.sitting)
            if is_correct is True:
                self.sitting.add_to_score(1)
                anon_progress.update_score(self.question, 1, 1)
            else:
                self.sitting.add_incorrect_question(self.question)
                anon_progress.update_score(self.question, 0, 1)
...
def signup(request, uuid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sitting = AnonSitting.objects.get(uuid=uuid)

            user.profile.score = sitting.current_score
            user.profile.start = sitting.start
            user.profile.end = sitting.end
